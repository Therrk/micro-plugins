...
function init()
    ...
    config.MakeCommand("squ", function() manipulate(".*", "'%1'", 1) end, config.NoComplete)
    config.MakeCommand("ang", function() manipulate(".*", "<%1>", 1) end, config.NoComplete)
    config.MakeCommand("base64dec", base64dec, config.NoComplete)
    config.MakeCommand("base64enc", base64enc, config.NoComplete)
    config.MakeCommand("dec", decNum, config.NoComplete)
    config.MakeCommand("inc", incNum, config.NoComplete)
    config.MakeCommand("rot13", rot13, config.NoComplete)
    config.MakeCommand("upp", function() manipulate("[%a]", string.upper) end, config.NoComplete)
    config.MakeCommand("low", function() manipulate("[%a]", string.lower) end, config.NoComplete)
    config.MakeCommand("rev", function() manipulate(".*", string.reverse) end, config.NoComplete)
